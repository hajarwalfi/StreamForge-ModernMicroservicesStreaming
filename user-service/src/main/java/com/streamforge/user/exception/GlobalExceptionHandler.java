...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleFeignException(FeignException ex) {
        ...
        if (ex.status() == 404) {
            body.put("error", "Not Found");
            body.put("message", "The requested video was not found in video-service. Please create the video first.");
        } else {
            body.put("error", status.getReasonPhrase());
            body.put("message", "Error communicating with video-service: " + ex.getMessage());
        }
